package org.djodjo.comm.jus.examples.nav;
 ...
 * <p/>
 * <p>To use <code>JToggle</code>, create one in your Activity and call through
 ...
 * has occurred.</p>
 * <p/>
 * <p><code>JToggle</code> can be used directly as a
 ...
 * ActionBar theme.
 */
public class JToggle implements DrawerLayout.DrawerListener {
    /**
     * Allows an implementing Activity to return an {@link JToggle.Delegate} to use
     * with JToggle.
     */
     ...
     *                                  for accessibility
     */
    public JToggle(Activity activity, DrawerLayout drawerLayout,
    ...
    /**
     * Construct a new JToggle with a Toolbar.
     * <p/>
     ...
     * <p/>
     * Please use {@link #JToggle(Activity, DrawerLayout, int, int)} if you are
     ...
     * animation.
     */
    <T extends Drawable & DrawerToggle> JToggle(Activity activity, Toolbar toolbar,
     ...
     * may have diverged in such a way that the JToggle was not notified.
...
// Reload drawables that can change with configuration
        ...
        if (enable != mDrawerIndicatorEnabled) {
            if (enable) {
                setActionBarUpIndicator((Drawable) mSlider,
                        mDrawerLayout.isDrawerOpen(GravityCompat.START) ?
                                mCloseDrawerContentDescRes : mOpenDrawerContentDescRes);
                ((DrawerArrowDrawableToggle) mSlider).setSmoothPosition(0f);
            } else {
                if (mHomeAsUpIndicator != null) {
                    setActionBarUpIndicator(mHomeAsUpIndicator, 0);
                } else {
                    setActionBarUpIndicator((Drawable) mSlider, 0);
                    ((DrawerArrowDrawableToggle) mSlider).setSmoothPosition(1f);
                }
            }
     ...
     * JToggle instance directly as your DrawerLayout's listener, you should call
    ...
    static class DrawerArrowDrawableToggle extends JDrawerArrowDrawable
        ...
        public void setSmoothPosition(float position) {
            float initPos = super.getProgress();
            ObjectAnimator animation = ObjectAnimator.ofFloat(this, "position", position);
    ...
    private static class HoneycombDelegate implements Delegate {
        ...
        JToggleHoneycomb.SetIndicatorInfo mSetIndicatorInfo;
        ...
        public Drawable getThemeUpIndicator() {
            return JToggleHoneycomb.getThemeUpIndicator(mActivity);
        }
        ...
        public void setActionBarDescription(int contentDescRes) {
            mSetIndicatorInfo = JToggleHoneycomb.setActionBarDescription(
