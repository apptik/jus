...
class OkHttpAsync implements HttpClient {
  ...
  @Override public void prepare(final States.GenericState state) {
    ...
    targetBacklog = state.targetBacklog;

    client = new OkHttpClient.Builder()
        .protocols(state.protocols)
        .dispatcher(new Dispatcher(new ThreadPoolExecutor(state.concurrencyLevel,
                state.concurrencyLevel, 60, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>())))
        .build();
